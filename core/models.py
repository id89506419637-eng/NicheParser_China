...
class Hypothesis:
    ...
    # Регуляторный риск — оценивает Agent 0B. Сигнальный флаг, НЕ юридическая
    # консультация: точный код ТН ВЭД и допуски выясняет таможенный брокер.
    # Значения: "none" | "dual_use" | "regulated" | "uncertain"
    #   none       — видимых регуляторных рисков нет
    #   dual_use   — возможное двойное назначение (высокоточные подшипники,
    #                дроны от 35кг, шифрование, рации, мульти-спектральные камеры)
    #   regulated  — особый контроль / сложная обязательная сертификация
    #                (медтехника → РЗН, продукты → Россельхознадзор, ТР ТС и т.п.)
    #   uncertain  — LLM не уверен, нужно ручное уточнение
    regulatory_risk: str = ""
