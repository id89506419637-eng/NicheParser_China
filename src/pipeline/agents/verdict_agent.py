...
ЗАДАЧА: для КАЖДОГО товара по индексу 0..{len(products)-1} выдай:
  ...
  reason:  1-2 коротких предложения, до 200 символов
...
МЯГКИЕ КОРРЕКТИРОВКИ (только если данные явно указывают):
- Объявлений на Авито >5000 при марже <70% — опусти вердикт на ступень.
- Поисковых запросов <500/мес — опусти вердикт на ступень.
- Объявлений на Авито <200 при марже 30-50% — можешь поднять до ВЕЗЁМ.
- Источник цены = «heuristic» — обязательно припиши «цена прикинута».

═══════════════ КРИТИЧЕСКИЕ ПРАВИЛА ФОРМУЛИРОВКИ REASON ═══════════════

НЕ используй оценочные слова без конкретных чисел.

ПЛОХО (так писать НЕЛЬЗЯ):                ХОРОШО (так писать НАДО):
  «высокий спрос»                          «1612 запросов/мес в Яндексе»
  «низкий спрос»                           «всего 80 запросов/мес»
  «свободная ниша»                         «47 объявлений на Авито»
  «насыщенный рынок»                       «5293 объявления на Авито»
  «хорошая конкуренция»                    «N продавцов на Авито»
  «востребован в РФ»                       «N запросов/мес, M объявлений»

НЕ употребляй слово «спрос» одно — пиши «поисковых запросов в Яндексе».
НЕ употребляй слово «конкуренция» одно — пиши «объявлений на Авито».

ВАЖНАЯ ЧЕСТНОСТЬ — что мы НЕ знаем:
- Wordstat показывает только число запросов в Яндексе, не покупок. Это сигнал
  поискового интереса, а не реального коммерческого спроса.
- Авито показывает объявления только в нём самом. Продажи на Wildberries,
  Ozon, Яндекс.Маркет мы НЕ проверяли — реальная конкуренция в РФ может быть
  выше.

Если данных мало или результат сомнительный — добавь в reason оговорку вида
«спрос на Авито не показатель — продаётся ли на WB/Ozon неизвестно»
или «всего N запросов в Яндексе — рынок может быть невидим».
...
{{
  "verdicts": [
    {{"index": 0, "verdict": "ВЕЗЁМ", "reason": "Маржа 92%, прибыль 4.2M₽ за MOQ. На Авито 5293 объявления, но 8500 запросов/мес в Яндексе — есть шанс."}},
    {{"index": 1, "verdict": "НЕ ВЕЗЁМ", "reason": "Маржа 12% — ниже порога 30%."}},
    {{"index": 2, "verdict": "ИЗУЧИТЬ", "reason": "Маржа 45%, но всего 80 запросов/мес в Яндексе — рынок может быть невидим, проверь WB/Ozon вручную."}}
  ]
}}
