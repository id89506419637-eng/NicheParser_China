...
ЗАДАЧА: по КАЖДОЙ гипотезе по индексу 0..{len(hypotheses)-1} выдай:
  ...
  deal_readiness: твоя оценка «готовности сделки» ЗА пользователя.
    Пользователь не знает нишу, поэтому ты сам отвечаешь ДА/НЕТ на 7 вопросов
    исходя из типа товара + короткое объяснение (до 80 символов) каждому.
    Формат: {{"q1":true, "q2":false, "q3":true, "q4":true, "q5":true,
             "q6":false, "q7":true,
             "why": {{"q1":"...", "q2":"...", "q3":"...", "q4":"...",
                     "q5":"...", "q6":"...", "q7":"..."}}}}
    Вопросы:
      q1 (демо):     Ценность товара понятна из фото+видео без физпоказа?
                     ДА если стандартный B2B-товар с ясными характеристиками;
                     НЕТ если нужно щупать, тестить, кастомизировать.
      q2 (гарантия): Есть ли реалистичный сценарий гарантии/замены (без
                     таскания в РФ-сервис)? ДА для типовых расходников/
                     запчастей с простым обменом; НЕТ для сложной электроники.
      q3 (предоплата): Клиент такого товара реально платит 100% предоплату
                     охотно (не сопротивляется — «пришлите фото после отгрузки»)?
                     ДА для дефицита, эксклюзива; НЕТ для стандарта в 5 магазинах.
      q4 (срок):     Срок поставки 30-45 дней приемлем для этой категории?
                     ДА для оборудования / реплик / промышленных товаров;
                     НЕТ для расходников которые нужны «завтра».
      q5 (юр.чистота): НЕТ нарушения торговой марки / патента? ДА для
                     небрендовых универсальных товаров; НЕТ для реплик
                     живых брендов, копий патентованных решений.
      q6 (расходники): Есть повторные покупки (расходники, обновления,
                     сопутствующие)? ДА даёт LTV, НЕТ = разовая продажа.
      q7 (5-10 объявл): Можно сделать несколько параллельных объявлений по
                     разным моделям товара? ДА для многовариантных категорий;
                     НЕТ для «один товар и всё».
  ...
  "critiques": [
    {{
      ...
      "deal_readiness": {{
        ...
        "q5": true, "q6": false, "q7": true,
        "why": {{
          "q1": "Оборудование сложное — нужен видео-обзор в работе, фото мало",
          "q2": "Типовой гарантийный сценарий, ЗИП высылается почтой",
          "q3": "После ухода Bosch клиенты платят предоплату охотно",
          "q4": "45 дней для промоборудования — стандарт, все привыкли",
          "q5": "Универсальная категория, торговых марок не касается",
          "q6": "Расходники минимальны, разовая покупка станка",
          "q7": "Есть 3-4 форм-фактора: под разные объёмы производства"
        }}
      }}
    }}
  ]
...
def _apply_critiques(hypotheses: List[Hypothesis], critiques: List[dict]) -> None:
    ...
    for i, h in enumerate(hypotheses):
        ...
        # Deal Readiness AI (2026-07-31): LLM отвечает на 7 вопросов за юзера.
        # Пользователь не знает нишу — эта оценка используется как дефолт если
        # ручной DR-чеклист не заполнен. Формат в БД: JSON строка с q1..q7 (0/1) + why.
        dr_ai = c.get("deal_readiness")
        if isinstance(dr_ai, dict):
            answers = {}
            for qn in ("q1", "q2", "q3", "q4", "q5", "q6", "q7"):
                v = dr_ai.get(qn)
                # Поддерживаем true/false/1/0/"yes"/"no"
                answers[f"{qn}_ai"] = 1 if v is True or v == 1 or (isinstance(v, str) and v.lower() in ("yes", "true", "да")) else 0
            why = dr_ai.get("why") or {}
            if isinstance(why, dict):
                # Обрезаем каждое обоснование до 160 символов
                answers["why"] = {k: str(v).strip()[:160] for k, v in why.items() if isinstance(k, str) and v}
            h.deal_readiness_ai = json.dumps(answers, ensure_ascii=False)
